#!/usr/bin/env py
"""A boolf*ck compiler that uses transpilation to C.

Using a C compiler to produce the actual machine code is kind of cheating, but
I don't want to write the assembly myself. This script parses boolf*ck source
...
class Instruction(StrEnum):
    """All seven boolf*ck instructions as a string enumeration.
    """
    MOVE_RIGHT = '>'
    MOVE_LEFT = '<'
    FLIP_BIT = '+'
...
# Table of optimizations that can be used to shorten boolf*ck programs
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog=argv[0],
        description='A boolf*ck compiler that uses transpilation to C.',
    )
...
def read_program(source_file: Path) -> Program:
    """Return a sequence of boolf*ck instructions from a given source file.
    """
...
def main(argv: list[str]) -> None:
    """Transpile boolf*ck source code to C and compile an executable."""
